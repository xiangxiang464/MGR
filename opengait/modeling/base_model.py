...
class BaseModel(MetaModel, nn.Module):
    ...
    @staticmethod
    def run_fused_test(models):
        """Accept a list of model instances and run the test loop, combining their predictions."""

        rank = torch.distributed.get_rank()
        combined_info_dict = None

        # Loop over all models
        for model in models:
            with torch.no_grad():
                # Get the inference results for this specific model
                info_dict = model.inference(rank)

            # Only on rank 0 do we gather and fuse results
            if rank == 0:
                if combined_info_dict is None:
                    combined_info_dict = info_dict.copy()
                    combined_info_dict['predictions'] = torch.zeros_like(info_dict['predictions'])

                # Accumulate predictions from multiple models
                combined_info_dict['predictions'] += info_dict['predictions']  # Simple sum, adjust for weighted fusion

        if rank == 0:
            loader = models[0].test_loader
            label_list = loader.dataset.label_list
            types_list = loader.dataset.types_list
            views_list = loader.dataset.views_list

            combined_info_dict.update({
                'labels': label_list, 'types': types_list, 'views': views_list})

            # Use model cfg from the first model for evaluation
            if 'eval_func' in models[0].cfgs["evaluator_cfg"].keys():
                eval_func = models[0].cfgs['evaluator_cfg']["eval_func"]
            else:
                eval_func = 'identification'
            eval_func = getattr(eval_functions, eval_func)
            valid_args = get_valid_args(
                eval_func, models[0].cfgs["evaluator_cfg"], ['metric'])
            try:
                dataset_name = models[0].cfgs['data_cfg']['test_dataset_name']
            except:
                dataset_name = models[0].cfgs['data_cfg']['dataset_name']

            # Average the predictions if needed
            combined_info_dict['predictions'] /= len(models)  # Averaging the predictions from all models

            return eval_func(combined_info_dict, dataset_name, **valid_args)
